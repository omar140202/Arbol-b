...
public class ArbolB {
    ...
    public void insertar(int valor, String palabra) {
        if (primerNodo.tengoHijos == false) {
            ...
            ingresarEnHijos(primerNodo, valor, palabra);
    ...
    public void ingresarEnHijos(Nodo conHijos, int valor, String palabra) {
        boolean entro = false;
        if (conHijos != null && !conHijos.tengoHijos) {
            ubicarValorEnArreglo(conHijos, valor, palabra);
            entro = true;
        }
        for (int i = 0; conHijos != null && i < 2 * grado + 1 && !entro; i++) {
            if (valor < conHijos.valores[i] || conHijos.valores[i] == 0) {
                entro = true;
                ingresarEnHijos(conHijos.nodo[i], valor, palabra);
                i = 2 * grado;
            }
        }
    }

    public void ubicarValorEnArreglo(Nodo nodoA, int valor, String palabra) {
        int cont = 0;
        while (cont <= 2 * grado) {
        ...
        for (int i = 0; i < 2 * grado + 1; i++) {
            ...
            for (int j = 0; nodo.nodo[i] != null && j < nodo.nodo[i].valores.length; j++) {
                if (nodo.nodo[i].valores[j] != 0) {
                    arbol += nodo.nodo[i].valores[j] +" "+ nodo.nodo[i].nombres[j] + ", ";
                }else{
                    if(j<2){
                        arbol += "Null,";
                    }else{
